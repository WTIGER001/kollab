...
func (m *mockUserRepository) UpdateProfile(ctx context.Context, id, email, displayName string) (*domain.User, error) {
	return nil, nil
}
...
func TestLocalSessionDoesNotResurrectDeletedAccount(t *testing.T) {
	repo := &mockUserRepository{}
	secret := []byte("test-session-secret-at-least-32-bytes")
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{"user_id": "deleted-user", "exp": time.Now().Add(time.Hour).Unix()})
	signed, err := token.SignedString(secret)
	if err != nil {
		t.Fatal(err)
	}
	handler := AuthMiddleware(secret, nil, repo)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { t.Fatal("deleted user reached handler") }))
	request := httptest.NewRequest("GET", "/api/teams", nil)
	...
	if len(repo.upserts) != 0 {
		t.Fatal("local token must never upsert users")
	}
}
