# Kollab Plugin Specification & Extensibility Architecture

This document defines the plugin model for Project Kollab. It details the technical boundaries for frontend-only and full-stack plugins, outlines the initial feature lineup, and contrasts them with popular Confluence equivalents.

---

## 1. High-Level Plugin Architecture

Kollab is designed from the ground up to support runtime-pluggable extensions. The architecture decouples the core document engine from plugin execution, ensuring security, performance, and cross-framework flexibility.

```
                  ┌──────────────────────────────────────────┐
                  │          Kollab Host Application          │
                  └─────┬──────────────────────────────┬─────┘
                        │                              │
...
Kollab supports two classes of plugins based on complexity, integration requirements, and security boundaries.
...
- **Execution Environment**: **Shadow DOM Web Components**. By wrapping the plugin inside a Shadow DOM, we ensure the plugin's CSS cannot pollute or break the parent Kollab application styles, and vice versa.
...
- **Backend Runtime**: **WebAssembly (Wasm) via wazero**. Wazero is a zero-dependency WebAssembly runtime written in pure Go. It allows Kollab to execute compiled Go, Rust, or TypeScript bytecode securely in an isolated sandbox.
- **Agentic Integration (LLM Tools)**: The Wasm manifest defines the plugin's REST/database access parameters and API methods. Kollab reads this schema and registers the plugin as a **Tool** for the inline LLM assistant, enabling the AI to run the plugin dynamically on user request.
...
## 3. Comparison Matrix: Kollab vs. Confluence Add-ons

Here is how Kollab replaces and improves upon the most popular enterprise Confluence add-ons:

| Confluence Add-on / Macro | Kollab Equivalent Plugin | Tech Model | Key UX Advantage in Kollab |
| :--- | :--- | :--- | :--- |
| **Draw.io Diagrams** | `kollab-drawio` | Frontend-Only | Fully offline drawing canvas, storing raw vectors directly inside the block JSON. |
| **Excalidraw Sketching** | `kollab-excalidraw` | Frontend-Only | Hand-drawn sketching canvas, running 100% locally with high performance and zero network dependency. |
| **Mermaid.js** | `kollab-mermaid` | Frontend-Only | Edit code on the left, watch live SVG re-renders on the right in real time. |
| **Jira Issues Macro** | `kollab-jira-sync` | Full-Stack | Fully integrated with LLM: write *"List open P0 bugs"* and let the AI trigger the Wasm tool to render the table. |
| **Chart / Table Filter** | `kollab-charts` | Frontend-Only | Connect charts directly to Kollab tables via block IDs. As you edit the table, the chart updates instantly. |
| **Excerpt / Include** | `kollab-excerpts` | Full-Stack | Re-use sections across documents. Go backend maps dependency hooks so updates to source blocks propagate automatically. |
| **Page Properties Report** | `kollab-properties` | Full-Stack | Index properties in Postgres. Use pgvector to query properties using natural language semantic searches. |

---

## 4. Deep Dive: Initial Plugin Lineup Specification

### 4.1 Draw.io (`kollab-drawio`)
...
### 4.2 Mermaid Diagrams (`kollab-mermaid`)
...
### 4.3 Interactive Tables (`kollab-tables`)
- **Category**: Formatting & Data
- **Type**: Frontend-Only
- **UX Flow**:
  - Replaces Confluence's basic rigid tables. Kollab tables support column type declarations (Text, Number, Date, Status Selectors, and User tags).
  - Renders calculations dynamically (e.g., column sums, averages) using lightweight spreadsheets math engines.

### 4.4 Image & Asset Manager (`kollab-image`)
- **Category**: Media
- **Type**: Full-Stack
- **UX Flow**:
...
### 4.5 Jira Issues Sync (`kollab-jira-sync`)
...
### 4.6 Dynamic Charts (`kollab-charts`)
...
### 4.7 Excerpts & Transclusions (`kollab-excerpts`)
...
### 4.8 Page Properties & Reporting (`kollab-properties`)
...
### 4.9 Excalidraw Sketching (`kollab-excalidraw`)
- **Category**: Visual Collaboration
- **Type**: Frontend-Only
- **UX Flow**:
...
Every Kollab UI plugin must export a class implementing the standard Custom Element lifecycle:

```typescript
interface KollabPluginElement extends HTMLElement {
  ...
  // Custom hook: Called by Kollab to fetch current plugin state JSON before saving page
  getPluginState(): Record<string, any>;
}
```

### 5.2 Go Backend Wasm Host Interface
The Go host defines standard WASI-like exports that the Wasm module can invoke to talk safely back to Kollab:
