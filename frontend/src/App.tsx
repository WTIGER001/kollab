...
export default function App({ isMockMode = false }: AppProps) {
  ...
  const isAuthenticated = isMockMode || !!auth?.isAuthenticated;
  
  const { data: teams = [] } = useTeams({ enabled: isAuthenticated });
  const { data: projects = [] } = useProjects("all", { enabled: isAuthenticated });
  const { data: systemSettings } = useSystemSettings({ enabled: isAuthenticated });
  ...
  useEffect(() => {
    if (!isMockMode && auth) {
      setOnUnauthorized(() => {
        ...
        setTimeout(() => auth.removeUser(), 1500);
      });
    }
  ...
  if (!isMockMode && auth?.error) {
    return <Box sx={{ p: 4, color: 'error.main' }}>Authentication Error: {auth.error.message}</Box>;
  }
  ...
  if (!isMockMode && !auth?.isAuthenticated) {
    ...
    return (
      <Box sx={{ display: 'flex', height: '100vh', width: '100vw', alignItems: 'center', justifyContent: 'center', bgcolor: 'background.default' }}>
        <Typography color="text.secondary">Redirecting to secure login...</Typography>
      </Box>
    );
  }
