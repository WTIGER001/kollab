...
const topics = [
  { title: "Settings and local accounts", text: "Open Server Settings from your account menu. Each section has its own page, with admin navigation replacing the workspace page tree. On a phone, use the menu button to open the admin drawer and choose a section. Use Save Changes before leaving a section. In local-account deployments, Users lets administrators create accounts, update profiles, reset passwords, and disable or remove accounts. Password changes and disabling an account invalidate existing sessions." },
  ...
  { title: "Deploying a release", text: "The deployment workflow verifies the code and builds release images in GitHub Actions. DigitalOcean downloads those images from GitHub Container Registry, replaces containers, and checks every API replica. It preserves data volumes and does not compile code or change package versions on the server. The build.sh script also supports local image builds. Confirm the public /api/health endpoint and sign-in after deployment, then verify external services separately." },
];
