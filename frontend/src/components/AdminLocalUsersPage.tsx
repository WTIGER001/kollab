...
export const AdminLocalUsersPage = () => {
  ...
  return <Box sx={{ p: { xs: 2, md: 5 }, maxWidth: 1100, width: "100%", color: "var(--text-primary)" }}>
    ...
    {loading ? <CircularProgress aria-label="Loading users" /> : <Stack spacing={1.5}>{users.map((user) => {
      ...
      return <Box key={user.id} sx={{ p: { xs: 2, md: 2.5 }, border: "var(--border-width) var(--border-style) var(--border-color)", borderRadius: "var(--border-radius-card)", bgcolor: "var(--panel-color)", boxShadow: "var(--shadow-elevation)" }}>
        <Stack direction={{ xs: "column", md: "row" }} spacing={2} sx={{ alignItems: { md: "center" }, justifyContent: "space-between" }}>
          <Box sx={{ minWidth: 190 }}><Stack direction="row" spacing={1} sx={{ alignItems: "center" }}><Typography sx={{ fontWeight: 750 }}>{user.displayName || user.username}</Typography><Chip size="small" label={user.isActive ? "Active" : "Disabled"} sx={{ bgcolor: user.isActive ? "color-mix(in srgb, var(--primary-color) 13%, transparent)" : "var(--glass-bg)", color: user.isActive ? "var(--primary-color)" : "var(--text-secondary)", fontWeight: 700 }} /></Stack><Typography variant="body2" sx={{ color: "var(--text-secondary)" }}>@{user.username}{user.email ? ` · ${user.email}` : ""}</Typography></Box>
